...
for run in range (0,max_run+1): #do it for each run
    ...
    for i in idxs1:
        i=int(i)
        values=np.loadtxt(save_folder+"/bands.{}.dat".format(i))
    ...
    s = s.replace("$K0", format(index-3)).replace("$K1", format(index+1))

    f = open(path_to_pp_inp+"/0.2_eV/pp.inp", "w")
    f.write(s)
    f.close()
    f = open(path_to_pp_inp+"/1.2_eV/pp.inp", "w")
    f.write(s)
    f.close()
