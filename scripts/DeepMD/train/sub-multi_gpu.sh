...
if [ ! -f tag_0_finished ] ;then  #if the file tag_0_finished exists, check the (possible) presence of checkpoints (for restart)
  { if [ ! -f model.ckpt.index ]; then CUDA_VISIBLE_DEVICES=0,1,2,3 horovodrun -np 4  dp train --mpi-log=master input.json; else CUDA_VISIBLE_DEVICES=0,1,2,3 horovodrun -np 4  dp train --mpi-log=master input.json --restart model.ckpt; fi }  1>> train.log 2>> train.log 
